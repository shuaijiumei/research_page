...
function loadPublications() {
  ...
  fetch(publicationsJsonPath)
      ...
      sortedYears.forEach((year) => {
        ...
        pubsByYear[year].forEach((pub) => {
          ...
          if (pub.link) {
            ...
            titleLink.className = "pub-title-text";
          ...
          // Highlight "Boyin Tan" with bold and underline
          const authorsText = pub.authors.replace(
            /Boyin Tan/g,
            "<strong><u>Boyin Tan</u></strong>"
          );
          line2.innerHTML = authorsText;
          ...
          else if (highlightText.toLowerCase().includes("spotlight"))
            badgeText = "Spotlight";
    ...
    .catch((error) => {
      ...
      publicationsList.innerHTML =
        "<p>Failed to load publications. Please check the console for details.</p>";
    });
}
...
function getVenueFullName(venueStr, year) {
  ...
  if (s.includes("TDSC"))
    return "IEEE Transactions on Dependable and Secure Computing";
  if (s.includes("TMC")) return "IEEE Transactions on Mobile Computing";
  if (s.includes("JSAC"))
    return "IEEE Journal on Selected Areas in Communications";
  if (s.includes("TGCN"))
    return "IEEE Transactions on Green Communications and Networking";
  if (s.includes("TNSE"))
    return "IEEE Transactions on Network Science and Engineering";
  if (s.includes("IoTJ") || s.includes("IoTJ"))
  ...
  if (s.includes("NeurIPS"))
    return `Annual Conference on Neural Information Processing Systems (NeurIPS${yearSuffix})`;
  if (s.includes("CVPR"))
    return `IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR${yearSuffix})`;
  if (s.includes("ICCV"))
    return `IEEE/CVF International Conference on Computer Vision (ICCV${yearSuffix})`;
  if (s.includes("ECCV"))
    return `European Conference on Computer Vision (ECCV${yearSuffix})`;
  if (s.includes("ICRA"))
    return `IEEE International Conference on Robotics and Automation (ICRA${yearSuffix})`;
  if (s.includes("AAAI"))
    return `AAAI Conference on Artificial Intelligence (AAAI${yearSuffix})`;
  if (s.includes("GLOBECOM"))
    return `IEEE Global Communications Conference (GLOBECOM${yearSuffix})`;
  if (s.includes("INFOCOM"))
    return `IEEE International Conference on Computer Communications (INFOCOM${yearSuffix})`;
  if (s.includes("MOBICOM"))
    return `Annual International Conference on Mobile Computing and Networking (MobiCom${yearSuffix})`;
